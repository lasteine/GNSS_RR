...
plt.figure()
swe_gnss.plot(linestyle='-', color='crimson', fontsize=12, figsize=(6, 5.5), ylim=(-50, 1000)).grid()
#fil.plot(linestyle='-', color='k', fontsize=12)
(swe).plot(color='k', linestyle='--')
manual2.SWE.plot(color='k', linestyle=' ', marker='+', markersize=8, markeredgewidth=2).grid()
...
(manual2.Density.dropna()).plot(color='steelblue', linestyle=' ', marker='*', markersize=8, markeredgewidth=2, label='Density (kg/m3)').grid()
plt.errorbar(manual2.index, manual2.SWE, yerr=manual2.SWE/10, color='k', linestyle='',capsize=4, alpha=0.5)
...
plt.legend(['GNSS', 'SHM', 'Manual', 'Accumulation_Manual (mm)', 'Accumulation_SHM (mm)', 'Density (kg/m3)'], fontsize=12, loc='upper left')
plt.xlim(datetime.datetime.strptime('2021-11-26', "%Y-%m-%d"), datetime.datetime.strptime('2022-05-01', "%Y-%m-%d"))
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.show()
#plt.savefig('data_neumayer/plots/SWE_Accts_NM_Emlid_30s_all_2021_22.png', bbox_inches='tight')
#plt.savefig('data_neumayer/plots/SWE_Accts_NM_Emlid_30s_all_2021_22.pdf', bbox_inches='tight')
...
(manual2.SWE.astype('float64')).plot(color='darkblue', linestyle=' ', marker='o', markersize=5, markeredgewidth=1).grid()
plt.errorbar((manual2.SWE.astype('float64')).index, (manual2.SWE.astype('float64')), yerr=(manual2.SWE.astype('float64'))/10, color='darkblue', linestyle='',capsize=4, alpha=0.5)
swe.plot(color='darkblue', linestyle=':', label='Accumulation (cm)').grid()
((dsh1*10)/1000*408).plot(color='darkgrey', linestyle='-').grid()
((dsh2*10)/1000*408).plot(color='lightgrey', linestyle='-').grid()
((dsh3*10)/1000*408).plot(color='darkgrey', linestyle=':').grid()
((dsh4*10)/1000*408).plot(color='lightgrey', linestyle=':').grid()
...
plt.xlabel(None)
plt.ylabel('SWE (mm w.e.)', fontsize=14)
plt.legend(['High-end GNSS', 'Low-cost GNSS', 'Manual (Pegelfeld Spuso)', 'Laser (SHM)', 'Buoy'], fontsize=12, loc='upper left')
plt.xlim(datetime.datetime.strptime('2021-11-26', "%Y-%m-%d"), datetime.datetime.strptime('2022-05-01', "%Y-%m-%d"))
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
#plt.show()
plt.savefig('data_neumayer/plots/SWE_all_2021_22.png', bbox_inches='tight')
plt.savefig('data_neumayer/plots/SWE_all_2021_22.pdf', bbox_inches='tight')
...
(dsh1.resample('D').median()-sh_leica.resample('D').median()).dropna().plot(color='darkgrey', linestyle='-').grid()
